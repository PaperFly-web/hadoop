...
public class HdfsClient {
	...
	public void init() throws URISyntaxException, IOException, InterruptedException {
		// 连接的集群nn地址
		URI uri = new URI("hdfs://linux1:8020");
		// 创建一个配置文件
		Configuration configuration = new Configuration();

		configuration.set("dfs.replication", "2");
		// 用户
		String user = "paperfly";
	...
	public void testPut() throws IOException {
		fs.copyFromLocalFile(false,true,new Path("D:\\ulog_HeadlightsCC_AppCrash__04caa03c-3c70-4191-82e7-70e7893f2235_0.xml"),new Path("/paperfly/1.xml"));
		// 参数解读：参数一：表示删除原数据； 参数二：是否允许覆盖；参数三：原数据路径； 参数四：目的地路径
//		fs.copyFromLocalFile(false, true, new Path("D:\\sunwukong.txt"), new Path("hdfs://linux1/xiyou/huaguoshan"));
	}
